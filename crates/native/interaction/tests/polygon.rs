...
//! desktop (apps/desktop/src/traces).
...
/// The newest object as a closed area: its corners relative to (E, N), and its bulges.
fn newest(b: &Bench) -> (Vec<[f64; 2]>, Option<Vec<f64>>) {
    let Entity::Polygon(path) = b.newest() else {
        panic!("a polygon, not {:?}", b.newest());
    };
    (
...
fn shift_turns_ortho_on_for_a_point() {
    ...
    b.click(0.0, 10.0);
    b.confirm();
    assert_eq!(newest(&b).0[1], [10.0, 0.0]);
}
