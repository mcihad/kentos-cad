...
use kentos_contracts::{
    ...
    DecimalMark, Entity, EntityBase, ExportReport, HeaderMode, ImportResult, LineError, PointEntity,
    TextEncoding, Vec2,
};
...
fn detect_delimiter(sample: &[&str]) -> CoordDelimiter {
    let candidates = [CoordDelimiter::Tab, CoordDelimiter::Semicolon, CoordDelimiter::Comma, CoordDelimiter::Space];
    ...
    for d in candidates {
        ...
        let steady = counts.iter().filter(|(n, _)| **n >= 2).map(|(_, c)| *c).max().unwrap_or(0);
...
fn detect_decimal(rows: &[Vec<String>], d: CoordDelimiter) -> DecimalMark {
    ...
    if comma > point { DecimalMark::Comma } else { DecimalMark::Point }
}

fn looks_like_header(first: &[String], rest: &[Vec<String>], comma: bool) -> bool {
    first.len() >= 2
        && first.iter().all(|f| !numeric(f, comma))
        && rest.iter().take(20).any(|r| r.iter().filter(|f| numeric(f, comma)).count() >= 2)
}
...
fn role_of_header(name: &str) -> Option<CoordColumn> {
    ...
    role_of_folded(&n)
        ...
        .or_else(|| name.split(|c: char| !c.is_alphanumeric()).find_map(|w| role_of_folded(&fold(w))))
}

fn role_of_folded(n: &str) -> Option<CoordColumn> {
    Some(match n {
        "y" | "saga" | "sagadeger" | "sagadegeri" | "easting" | "east" | "e" | "dogu" => CoordColumn::Y,
        "x" | "yukari" | "yukarideger" | "yukaridegeri" | "northing" | "north" | "n" | "kuzey" => CoordColumn::X,
        "z" | "h" | "kot" | "yukseklik" | "elevation" | "elev" | "height" => CoordColumn::Z,
        "ad" | "adi" | "nokta" | "noktaadi" | "noktano" | "noktanumarasi" | "no" | "name" | "point" | "pt" | "id" => CoordColumn::Name,
...
fn suggest(header: Option<&[String]>, rows: &[Vec<String>], ncols: usize, comma: bool) -> Suggestion {
    let column_values = |c: usize| -> Vec<f64> { rows.iter().filter_map(|r| r.get(c)).filter_map(|f| parse_decimal(f, comma)).collect() };
    // Numeric when most of its fields are numbers (a few bad lines must not hide a coordinate column).
    let is_numeric = |c: usize| {
        let present = rows.iter().filter(|r| r.get(c).is_some_and(|f| !f.is_empty())).count();
    ...
    if let Some(names) = header {
        let mut cols: Vec<CoordColumn> = (0..ncols).map(|c| names.get(c).and_then(|n| role_of_header(n)).unwrap_or(CoordColumn::Skip)).collect();
        ...
        if let (Some(y), Some(x)) = (y, x) {
            ...
            return Suggestion { columns: cols, hint };
        }
    }
    ...
    let integer_names = rows.iter().filter_map(|r| r.first()).all(|f| !f.contains(['.', ',']));
    ...
    if nums.len() < 2 {
        return Suggestion { columns: cols, hint: Some("İki sayısal sütun bulunamadı; Y (sağa) ve X (yukarı) sütunlarını elle seçin.".to_string()) };
    }
    ...
    if let Some(c) = (1..ncols).find(|&c| cols[c] == CoordColumn::Skip && !numeric_cols.contains(&c)) {
        cols[c] = CoordColumn::Code;
    }
    Suggestion { columns: cols, hint }
}

/// Why a row is not a point.
enum RowError {
    Missing { role: CoordColumn, need: usize, have: usize },
    Empty { role: CoordColumn },
    NotNumber { role: CoordColumn, value: String },
}
...
impl RowError {
    fn generic(&self) -> String {
        match self {
            ...
            RowError::NotNumber { role, .. } => format!("{} değeri sayı değil", column_label(*role)),
        }
    }
    ...
    fn detail(&self) -> String {
        match self {
            RowError::Missing { role, need, have } => {
                format!("{have} alan var; {} için en az {need} alan gerekiyor.", column_label(*role))
            }
            RowError::Empty { role } => format!("{} değeri boş.", column_label(*role)),
            RowError::NotNumber { role, value } => format!("{} değeri “{value}” sayı değil.", column_label(*role)),
        }
    }
}
...
fn read_row(fields: &[String], cols: &[CoordColumn], comma: bool) -> Result<Row, RowError> {
    ...
    let number = |role: CoordColumn| -> Result<Option<(f64, String)>, RowError> {
        ...
        match fields.get(i).map(String::as_str) {
            ...
            None => Err(RowError::Missing { role, need: i + 1, have: fields.len() }),
            Some("") => Err(RowError::Empty { role }),
            Some(f) => parse_decimal(f, comma)
                .map(|v| Some((v, if comma { f.replace(',', ".") } else { f.to_string() })))
                .ok_or_else(|| RowError::NotNumber { role, value: f.to_string() }),
        }
    };
    let y = number(CoordColumn::Y)?;
    let x = number(CoordColumn::X)?;
    let z = number(CoordColumn::Z)?;
    let text = |role: CoordColumn| find(role).and_then(|i| fields.get(i)).map(|s| s.trim().to_string()).filter(|s| !s.is_empty());
    match (y, x) {
        (Some((y, _)), Some((x, _))) => Ok(Row { name: text(CoordColumn::Name).unwrap_or_default(), y, x, z, code: text(CoordColumn::Code) }),
        // Both roles exist whenever this is reached (checked before the rows are read).
        _ => Err(RowError::Missing { role: CoordColumn::Y, need: 1, have: fields.len() }),
    }
}
...
pub fn read(bytes: &[u8], opts: &CoordReadOptions) -> CoordRead {
    ...
    let lines: Vec<(u32, &str)> = body
        .split('\n')
        .enumerate()
        .map(|(i, l)| (u32::try_from(i + 1).unwrap_or(u32::MAX), l.strip_suffix('\r').unwrap_or(l)))
        .filter(|(_, l)| !is_comment(l))
        .collect();
    let sample: Vec<&str> = lines.iter().take(SAMPLE).map(|(_, l)| *l).collect();

    let delimiter = if opts.delimiter == CoordDelimiter::Auto { detect_delimiter(&sample) } else { opts.delimiter };
    ...
    let header = match opts.header {
        HeaderMode::Auto => sample_rows.first().is_some_and(|first| looks_like_header(first, &sample_rows[1..], comma)),
        HeaderMode::Yes => !sample_rows.is_empty(),
        HeaderMode::No => false,
    };
    let header_fields = if header { sample_rows.first().cloned().unwrap_or_default() } else { Vec::new() };
    let data_sample = if header { &sample_rows[1.min(sample_rows.len())..] } else { &sample_rows[..] };
    ...
    let steady = counts.iter().max_by_key(|(n, c)| (**c, **n)).map_or(0, |(n, _)| *n);
    let ncols = steady.max(header_fields.len());

    let mut hints = Vec::new();
    let columns: Vec<CoordColumn> = if opts.columns.is_empty() {
        let s = suggest(header.then_some(header_fields.as_slice()), data_sample, ncols, comma);
        hints.extend(s.hint);
        s.columns
    } else {
        (0..ncols.max(opts.columns.len())).map(|i| opts.columns.get(i).copied().unwrap_or(CoordColumn::Skip)).collect()
    };
    ...
    for role in [CoordColumn::Name, CoordColumn::Y, CoordColumn::X, CoordColumn::Z, CoordColumn::Code] {
        if columns.iter().filter(|c| **c == role).count() > 1 {
            hints.push(format!("{} birden çok sütuna verilmiş; yalnızca ilki okunur.", column_label(role)));
        }
    }
    ...
    for (k, (line, text)) in lines.iter().enumerate().skip(usize::from(header)) {
        let fields = if k < sample_rows.len() { sample_rows[k].clone() } else { split(text, delimiter) };
        let row = if usable { Some(read_row(&fields, &columns, comma)) } else { None };
        let error = match &row {
            Some(Err(e)) => {
                error_count += 1;
                report.skip("Satır", &e.generic(), *line);
                if errors.len() < ERRORS_KEPT {
                    errors.push(LineError { line: *line, message: format!("Satır {line}: {}", e.detail()) });
                }
                Some(e.detail())
            }
            _ => None,
        ...
        if preview.len() < opts.preview_rows as usize {
            preview.push(CoordRow { line: *line, fields, error });
        }
        ...
        let b = bounds.get_or_insert(Bounds { min_x: row.y, min_y: row.x, max_x: row.y, max_y: row.x });
    ...
    if usable && points > 0 {
        ...
        let fine = ys.iter().chain(&xs).filter(|v| (**v * 1000.0).fract() != 0.0).count();
        if ys.iter().all(|v| v.abs() <= 180.0) && xs.iter().all(|v| v.abs() <= 90.0) && fine * 10 >= (ys.len() + xs.len()) * 8 {
    ...
    if duplicate_names > 0 {
        report.note_n("Aynı adlı nokta", "başka bir noktanın adını taşıyor; ikisi de alınır", 0, duplicate_names);
    }
    if lines.is_empty() {
    ...
    CoordRead {
        ...
        hints,
        bounds,
        result: opts.entities.then(|| ImportResult { entities, layers: Vec::new(), report: report.import(), bounds }),
    }
}
...
pub fn write(input: &CoordWriteInput) -> (Vec<u8>, ExportReport) {
    ...
    let columns: Vec<CoordColumn> = input.columns.iter().copied().filter(|c| *c != CoordColumn::Skip).collect();
    let quote = |s: &str| -> String {
        match delimiter_char(input.delimiter) {
            Some(d) if s.contains(d) || s.contains('"') => format!("\"{}\"", s.replace('"', "\"\"")),
            Some(_) => s.to_string(),
            None if s.chars().any(char::is_whitespace) => s.split_whitespace().collect::<Vec<_>>().join("_"),
            None => s.to_string(),
        }
    };
            ...
            .map(|c| match c {
                CoordColumn::Name => {
                    ...
                    if q != p.name && delimiter_char(input.delimiter).is_none() {
                        report.note("Nokta adı", "adındaki boşluklar “_” oldu (NCN'de alanlar boşlukla ayrılır)", 0);
                    }
                    q
                }
    ...
    let bytes = match input.encoding {
        ...
        TextEncoding::Windows1254 => {
            ...
            report.note_n("Karakter", "Windows-1254'te olmadığı için “?” yazıldı; UTF-8 seçerek koruyabilirsiniz", 0, lost);
            bytes
        }
    };
    (bytes, report.export())
...
mod tests {
    ...
    fn opts() -> CoordReadOptions {
        CoordReadOptions { preview_rows: 10, entities: true, ..Default::default() }
    }
            ...
            .map(|res| {
                res.entities
                    ...
                    .filter_map(|e| match e {
                        Entity::Point(p) => Some((p.base.label.clone().unwrap_or_default(), p.p.x, p.p.y, p.z)),
                        _ => None,
                    })
                    .collect()
            })
    ...
    fn netcad_order_with_spaces_and_numeric_names() {
        let r = read(b"1001 452345.123 4412345.678 105.2\n1002   452346.5\t4412346.25 106\n", &opts());
        ...
        assert_eq!(
            points(&r),
            vec![("1001".into(), 452345.123, 4412345.678, Some(105.2)), ("1002".into(), 452346.5, 4412346.25, Some(106.0))]
        );
    ...
    fn a_turkish_spreadsheet_with_semicolons_and_decimal_commas() {
        let r = read("Nokta;Sağa;Yukarı;Kot\r\nP1;452345,12;4412345,5;12,75\r\nP2;452346;4412346;\r\n".as_bytes(), &opts());
        ...
        assert_eq!(r.columns, vec![CoordColumn::Name, CoordColumn::Y, CoordColumn::X, CoordColumn::Z]);
        assert_eq!(points(&r), vec![("P1".into(), 452345.12, 4412345.5, Some(12.75)), ("P2".into(), 452346.0, 4412346.0, None)]);
    ...
    fn csv_with_quotes_comments_and_a_code_column() {
        ...
        let Entity::Point(p) = &res.entities[1] else { panic!() };
        assert_eq!(p.base.attrs.get(ATTR_CODE).map(String::as_str), Some("ağaç \"çınar\""));
        let Entity::Point(p) = &res.entities[0] else { panic!() };
    ...
    fn bad_lines_are_reported_with_their_numbers() {
        ...
        assert_eq!(r.errors[0].message, "Satır 2: Y (sağa) değeri “abc” sayı değil.");
        assert_eq!(r.errors[1].message, "Satır 3: 2 alan var; X (yukarı) için en az 3 alan gerekiyor.");
        assert_eq!(r.preview[1].error.as_deref(), Some("Y (sağa) değeri “abc” sayı değil."));
    ...
    fn sizes_tell_northings_from_eastings() {
        ...
        let r = read(b"4412345.678 452345.123 100.5\n4412346.678 452346.123 101.5\n", &opts());
        assert_eq!(r.columns, vec![CoordColumn::X, CoordColumn::Y, CoordColumn::Z]);
        assert_eq!(points(&r)[0], (String::new(), 452345.123, 4412345.678, Some(100.5)));
        assert_eq!(r.hints.len(), 1);
        // Chosen by hand the other way round, the reader says what it sees.
        let r = read(b"4412345.678 452345.123 100.5\n", &CoordReadOptions { columns: vec![CoordColumn::Y, CoordColumn::X, CoordColumn::Z], ..opts() });
        assert!(r.hints.iter().any(|h| h.contains("yer değiştirmiş")), "{:?}", r.hints);
    }
    ...
    fn exact_values_survive_writing_and_reading() {
        let pts = vec![
            kentos_contracts::CoordPoint { name: "P 1".into(), p: Vec2 { x: 452345.123, y: 4412345.678 }, z: Some(0.1 + 0.2), code: None },
            kentos_contracts::CoordPoint { name: "Ağaç".into(), p: Vec2 { x: 1.0 / 3.0, y: -2.5e-7 }, z: None, code: None },
        ];
        for (delimiter, encoding) in [
            ...
            let input = CoordWriteInput {
                points: pts.clone(),
                delimiter,
                columns: vec![CoordColumn::Name, CoordColumn::Y, CoordColumn::X, CoordColumn::Z],
                header: delimiter != CoordDelimiter::Space,
                encoding,
            };
            let (bytes, rep) = write(&input);
            assert_eq!(rep.counts.get("point"), Some(&2));
            let back = read(&bytes, &CoordReadOptions { columns: input.columns.clone(), ..opts() });
            ...
            assert_eq!(got[0].0, if delimiter == CoordDelimiter::Space { "P_1" } else { "P 1" });
        }
    }
}
