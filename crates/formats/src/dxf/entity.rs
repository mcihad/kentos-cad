...
use super::hatch::{parse_hatch, Hatch};
...
pub enum Kind {
    ...
    Arc { c: P3, r: f64, a0: f64, a1: f64 },
    Ellipse { c: P3, major: P3, ratio: f64, t0: f64, t1: f64 },
...
fn bad(reason: &str) -> Unreadable {
    Unreadable { reason: reason.to_string() }
}
...
impl<'g, 'a> Groups<'g, 'a> {
    ...
    pub fn num(&self, code: i32) -> Result<Option<f64>, Unreadable> {
        match self.list.iter().find(|p| p.code == code) {
            None => Ok(None),
            Some(p) => parse_real(p.text()).map(Some).ok_or_else(|| bad(&format!("sayı okunamadı (grup {code}, satır {})", p.line))),
        }
    }
    ...
    pub fn int(&self, code: i32) -> i64 {
        self.list.iter().find(|p| p.code == code).and_then(|p| parse_int(p.text())).unwrap_or(0)
    }
    ...
    pub fn string(&self, code: i32) -> String {
        self.list.iter().find(|p| p.code == code).map(|p| self.dec.string(p.value)).unwrap_or_default()
    }

    /// A point from its x code (y is x + 10, z is x + 20).
    pub fn point(&self, x: i32) -> Result<Option<P3>, Unreadable> {
        let Some(px) = self.num(x)? else { return Ok(None) };
        Ok(Some([px, self.num_or(x + 10, 0.0)?, self.num_or(x + 20, 0.0)?]))
    }
...
fn points(list: &[Pair<'_>], x: i32) -> Result<Vec<P3>, Unreadable> {
    ...
    for p in list {
        let v = || parse_real(p.text()).ok_or_else(|| bad(&format!("sayı okunamadı (grup {}, satır {})", p.code, p.line)));
        if p.code == x {
            out.push([v()?, 0.0, 0.0]);
...
fn reals(list: &[Pair<'_>], code: i32) -> Result<Vec<f64>, Unreadable> {
    list.iter()
        ...
        .map(|p| parse_real(p.text()).ok_or_else(|| bad(&format!("sayı okunamadı (grup {code}, satır {})", p.line))))
        .collect()
}
...
pub fn parse(name: &str, line: u32, list: &[Pair<'_>], dec: Decoder, after: Vec<(u32, Vec<Pair<'_>>)>, fit_data_in_hatch_splines: bool) -> Result<Parsed, Unreadable> {
    ...
    let kind = match name {
        ...
        "POINT" => Kind::Point { p: g.point_or_zero(10)? },
        "CIRCLE" => Kind::Circle { c: g.point_or_zero(10)?, r: g.num_or(40, 0.0)? },
        "ARC" => Kind::Arc { c: g.point_or_zero(10)?, r: g.num_or(40, 0.0)?, a0: g.num_or(50, 0.0)?, a1: g.num_or(51, 360.0)? },
        ...
        "LWPOLYLINE" => {
            ...
            Kind::LwPolyline { pts, bulges, closed: g.int(70) & 1 == 1, elevation: g.num_or(38, 0.0)?, width }
        }
        "POLYLINE" => {
            ...
            Kind::Polyline { flags: g.int(70), verts, elevation: g.point_or_zero(10)?[2], width }
        }
        "SPLINE" => Kind::Spline {
            flags: g.int(70),
            degree: g.int(71),
        ...
        "SOLID" | "TRACE" | "3DFACE" => {
            ...
            Kind::Face { pts: [a, b, c, d], solid: name != "3DFACE" }
        }
        "INSERT" => {
        ...
        "DIMENSION" | "ARC_DIMENSION" | "LARGE_RADIAL_DIMENSION" => Kind::Block { block: g.string(2), what: "Ölçü (DIMENSION)" },
        "ACAD_TABLE" => Kind::Block { block: g.string(2), what: "Tablo (ACAD_TABLE)" },
        "XLINE" | "RAY" => Kind::Xline { p: g.point_or_zero(10)?, dir: g.point_or_zero(11)?, ray: name == "RAY" },
        "LEADER" => Kind::Leader { pts: points(list, 10)? },
        other => Kind::Unsupported(other.to_string()),
    };
    Ok(Parsed { kind, common, line, name: name.to_string() })
}
