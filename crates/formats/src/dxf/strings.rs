...
use crate::text::{decode, Encoding};
...
impl Decoder {
    pub fn string(&self, raw: &[u8]) -> String {
        ...
        if s.contains("\\U+") || s.contains("\\u+") { unescape_unicode(&s) } else { s }
    }
}
...
pub fn mtext_lines(s: &str) -> Vec<String> {
    ...
    let mut i = 0;
    let skip_to_semicolon = |from: usize| chars[from..].iter().position(|&c| c == ';').map_or(chars.len(), |k| from + k + 1);
    while i < chars.len() {
        let c = chars[i];
        ...
        match (c, chars.get(i + 1).copied(), cur) {
            ('\\', Some(k), Some(cur)) => match k {
                ...
                'S' => {
                    ...
                    let body: String = chars[i + 2..end.min(chars.len())].iter().filter(|&&c| c != ';').map(|&c| if c == '^' || c == '#' { '/' } else { c }).collect();
                    cur.push_str(body.trim());
                    i = end;
                }
                'A' | 'C' | 'c' | 'F' | 'f' | 'H' | 'h' | 'Q' | 'q' | 'T' | 't' | 'W' | 'w' | 'p' => i = skip_to_semicolon(i + 2),
                _ => {
                    cur.push(k);
                    i += 2;
                }
    ...
    lines.into_iter().map(|l| text_codes(l.trim_end())).collect()
}
...
mod tests {
    ...
    fn turkish_code_page_and_unicode_escapes() {
        let d = Decoder { enc: Encoding::Windows1254 };
        assert_eq!(d.string(b"\xDDstasyon \xFEev"), "İstasyon şev");
        assert_eq!(d.string(b"Ada \\U+0130\\U+015F x"), "Ada İş x");
        let u = Decoder { enc: Encoding::Utf8 };
        assert_eq!(u.string("Ağaç".as_bytes()), "Ağaç");
    }

    #[test]
    fn text_control_codes() {
        assert_eq!(text_codes("45%%d  %%p0.05  %%c20  100%%%  %%uAltı%%u çizili %%065"), "45°  ±0.05  Ø20  100%  Altı çizili A");
    ...
    fn mtext_formatting_is_removed() {
        ...
        assert_eq!(mtext_lines(s), vec!["Parsel 12", "Alan: 450 m2/", "• Madde 1 {sabit}"]);
