...
- **Şeritte son seçilen yöntemin hatırlanması (sahibin sorusu).** Web, Daire ▾ ve Yay ▾ düğmesinin üstünde son seçilen yöntemi tutar ve oturumlar arasında hatırlar. Masaüstünde yerleşim kalıcılığı henüz yoktur. Seçenekler:
  ...
  - **Varsayılan (26 Eylül, birleştirmede):** önerilen seçenek; düğme ilk yöntemi başlatır, menü hepsini verir. Web'in Kot noktası aracı `kot` katmanı yoksa uyarır (ikinci soru; önerilen, bugünkü durum). Sahip başka birini seçerse değişir.
