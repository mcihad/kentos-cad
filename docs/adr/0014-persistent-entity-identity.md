...
- **Web açılışı** (`app/fileIO.ts`):
  ...
  - **Neden biçim modülü:** sözleşmenin tipli okuyucusu, kanonik yazıcı ve sha1/sha2 biçim modülünü 676 099'dan 934 257 bayta büyüttü (+258 KB; brotli 203 548 → 254 977, +51 KB). Aynı kod geometri çekirdeğine konsa, başlangıçta yüklenen modül 1 169 816'dan 1 505 531 bayta çıkardı (+336 KB; brotli 312 504 → 392 490, +80 KB), her açılışta. Biçim modülü yalnız dosya açılınca, içe ya da dışa aktarılınca yüklenir, worker'da çalışır ve sayfayı bekletmez. İstemcisi de içe ve dışa aktarma pencereleri gibi ilk kullanımda yüklenir. Ölçüm: `--profile wasm`, wasm-bindgen 0.2.128; brotli 11 ve gzip 9 Node'un zlib'iyle (`vite.config.mjs`'in yaptığı gibi).
    - Opak anahtarlar dosya sırasında kalırsa (serde_json `preserve_order`, hiçbir derlemede açık değil) işlev çizimin kopyasını sıralar. Masaüstü belgesi paylaşılan çizimi verdiği için kopya gerekir; bu kod ölçüme dahildir (yaklaşık 14 KB).
