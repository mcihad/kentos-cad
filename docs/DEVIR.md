...
- **Dosya biçimleri** (`wip/formats-dxf` dalından, `33f9981` … `96f4460`): koordinat listesi (Netcad NCN, TXT, CSV) içe/dışa aktarma ve DXF içe aktarma Rust'ta (`crates/shared/formats`, ayrı ve yalnız komutla yüklenen WASM paketi `apps/web/src/io/pkg`). ADR 0009 kabul edildi (24 Eylül). **DXF dışa aktarma** da Rust'ta (`51a13a7`, `9f1c383`, `717f2f2`; alt ajan, ADR 0009 “DXF yazma”): AutoCAD 2007 ASCII DXF (AC1021; UTF-8, metre); KentOS'a aynı nesneler olarak, koordinatlar bit bit döner. Ölçüler bunun dışındadır: çizgi, yay ve yazıya patlatılır. DXF'in tutamadığı etiket, öznitelik, sembol ve tema renkleri `KENTOS` genişletilmiş verisiyle (XDATA) gider. Biçim paketi (yalnız içe/dışa aktarmada yüklenir) 493 639 → 663 791 bayt, gzip -9 187 224 → 247 338 (~3 KB'ı `ring_contains`'in artık kullandığı sağlam yüklemden).
- **WASM paketi:** 894 506 bayt, gzip 302 107 bayt (sağlam kararlar R1–R2b ve paketli dönüşümle; önce 883 626 / 297 902; DXF yazıcısı değiştirmedi). Başlangıç sınırı kullanıcı kararıyla 300'den 350 KB gzip'e yükseltildi (24 Eylül, ADR 0005 “Değişiklikler”); işlev adları pakette kalır. Her çekirdek diliminden sonra boyut ADR 0008'e yazılır.
- **Son doğrulama** (`main`, DXF dışa aktarma, 24 Eylül, bulut konteyneri): `pnpm typecheck` temiz; `pnpm test` 719 test geçti, 7 atlandı (fixture kaydedicileri ve istekle çalışan dönüşüm ölçümü); `pnpm build` başarılı (dışa aktarma penceresi ayrı parça, 6,4 kB); `pnpm rust:test` temiz (278 test, clippy `-D warnings`; veritabanı testleri sır olmadığı için atlandı); `cargo fmt --all -- --check` temiz; `pnpm e2e` 124 denetim geçti, düşen üçü bu konteynerde düşen WebGPU denetimleri (§5; şeritten önceki `6505a6e` de aynı piksel sayılarıyla düşüyor). Alt ajan ayrıca örnek bir dosyayı ezdxf 1.4 ile denetledi (hata yok). Tam takımda `ellipse.test.ts`'in yoğun örnekleme denetimi makine yüklüyken bir kez 5 sn sınırını aştı; kendi süresi 30 sn yapıldı (`67c663d`).
- **Ölçüm:**
...
3. **§23.3 sağlam geometrik kararlar** (robust predicates; kullanıcı 24 Eylül'de başlattı). Yalnız Rust'ta, yeni bağımlılık eklemeden. Yapılanlar (ADR 0008 “Sağlam kararlar”): R1 `predicates::orient2d` (Shewchuk'un uyarlamalı yüklemi) ve bağımsız referans (`geometry_call_reference.py`, 85 kesin işaret), donmuş çağrılar `calls-r1-predicates.json` (`6ef7e39`); R2a nokta-halka (`95ec850`); R2b sarım sayıları, açı toplamı ve bindirmenin ışın dizini (`83b11a4`). Hiçbir donmuş durum değişmedi; her dilimin testi eski kuralın yanıldığı noktaları içerir. Kalanlar, her biri ayrı dilim:
   - **Halka izlemede köşe çevresindeki sıra** (`overlay.rs` `trace`, `arrangement.rs` `leave_angle`): parçalar kirişin 1e-4'ündeki noktanın atan2 açısıyla sıralanıyor; neredeyse aynı yöne çıkan iki parça yanlış sıralanabilir. Düz parçalar için kesin karşılaştırıcı (varış yönüne göre yarı düzlem ve `orientation`) kurulabilir; yay için teğet ve eğrilik gerekir (teğet sin/cos'tan geldiği için kesin değildir): önce düz-düz, sonra düz-yay ayrımı.
   - **Kesişim kararları** (`intersect.rs` `seg_seg`, `line_line`): 1e-9 parametre toleransı ve paralellik sınırı CAD anlamıdır (değen sayılır), korunur; kesin yüklem yalnız tolerans bandının dışındaki kesin kesişir/kesişmez kararını verebilir. Golden farkı beklenir, okunarak yazılır.
   - **incircle** (Shewchuk): TIN/eşyükselti gelince (Delaunay); şimdi kullanan yok, eklenmedi.
   - Kurallar: her değişiklikte kaydediciyle yeniden kayıt ve farkın satır satır okunması (§4); sabit toleransları (1e-9, `TOL = 1e-6`) büyütmek yasak (CLAUDE.md §23.4).
4. **Taşıma ve kopyalamada geri alma adımı:** dönüşüm depoda yapılır, sonuç paketli döner (`e81a616`, alt ajan; ADR 0008 “Taşı, kopyala ve yapıştır depodan”): 10 000 karışık TM nesnesinde dönüşüm p50 294 → 18 ms, komut ~310 → 35–60 ms. Komutun kalan süresi belgenin geri alma adımıdır (nesne başına `update`/`add` ve olaylar; 10 000 nesnede ~20–40 ms): `addMany` gibi toplu bir `updateMany` sonraki iyileştirmedir.
5. **Katmanlar paneli, sanallaştırma:** sayıların yerinde yazılması yapıldı (`a3c7c5d`, `1fc00f4`, alt ajan; CLAUDE.md §4.10, §6.3): düzenleme, geri alma ve yinelemede panel ağacı kurmaz; nesne sayıları ve katman durumu satırlara yerinde yazılır, satırlar aynı öğe kalır; ağaç yalnız biçimi değişince (`structure`, yeni `expanded` olayı) görev başına bir kez kurulur. Kalan: `TreeView` sanallaştırması (katmanlar, işlemler, stil yöneticisi ortak). Görünür satırların düz listesi veride tutulur, yalnız kaydırma penceresindeki satırlar kurulur; satır boyu `--row-h` × `--ui-scale`'den ölçülür ve yazı ölçeği değişince yeniden ölçülür (şerit gibi); `focus` ve `rowOf` satırı önce görünüme getirir (klavye, yeniden adlandırma); satırlara `aria-setsize`/`aria-posinset` yazılır; satır kaldırılınca haber veren bir kanca `LayersPanel.rows`'u kurulu satırlarla sınırlar; biçim değişikliğinde satırları kimliğe göre yeniden kullanmak aynı işin parçasıdır (bugün 317 satırda ~20 ms + ~40 ms stil ve yerleşim).
6. **DXF dışa aktarma yapıldı** (ADR 0009 “DXF yazma”). Kalanlar:
   - dosyanın gerçek AutoCAD, Netcad, BricsCAD ve QGIS'te açılması: yalnız ezdxf ile denetlendi, asıl risk bu;
   - büyük çizimde yazma süresi (nesneler sayfada toplanıp worker'a kopyalanır, JSON geçer) ve büyük dosyada içe aktarmanın ana iş parçacığındaki süresi; ikisi de ölçülmedi;
   - istenirse gerçek DIMENSION (anonim blok ve ölçü stiliyle) ve MTEXT;
   - ölçünün patlatılması kendi çekirdek işlevine alınırsa biçim paketi ~5 KB küçülür;
