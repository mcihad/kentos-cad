...
- **Kapsam: yalnız hesap Rust'ta, arayüz TypeScript'te kalır.** Kullanıcı bunu açıkça sordu ve doğruladı; bu ayrımı koruyun.
  - Rust'ta (`crates/shared/geometry-core` ve `style-core`, tarayıcıda WASM): ifade dili (derleme, toplu değerlendirme, JavaScript'in sayı ve metin anlamı, Türkçe sıralama; `crates/shared/style-core`), geometri işlemleri (kesişim, budama, uzatma, öteleme, köşe yuvarlama, alan cebiri ve bindirme, yay/elips/eğri, ölçü yerleşimi, tarama çizgileri, üçgenleme), nesne ölçüleri, geometri deposu (seçme, kenet, pencere seçimi, etiket ve tutamaç kararları, araç önizlemeleri, çizilecek geometri, ifadelerin geometri değerleri), işlem araçlarının geometrisi (köşe numaralama, kenar ölçüleri), araçların yapı hesapları (nokta girişi, orto/kutupsal, nesne izleme), dosya biçimleri (`crates/shared/formats`), sunucunun PostGIS geometrisi (tessellate, EWKB), §23 sayısal politika.
  - TypeScript'te: bütün arayüz (DOM, paneller, pencereler, menüler, komutlar, kısayollar), araçların akışı (tıklama, istem, önizlemenin çizimi), belge modeli ve geri alma, çizim motorları (WebGL2/WebGPU), kamera ve ekran pikseli hesapları, bulut eşitleme, stil motoru ve SVG düzenleyicisi. Bu ikisinin geometrisi `style-core`'un sıradaki dilimleridir (§3 madde 7).
...
- **WASM paketi:** 1 008 667 bayt, gzip 349 550 bayt (ifade diliyle; önce 898 018 / 303 940). Başlangıç sınırı kullanıcı kararıyla 300'den 350 KB gzip'e yükseltildi (24 Eylül, ADR 0005 “Değişiklikler”); işlev adları pakette kalır. **Sınıra 0,45 KB kaldı**: stil motorunun geometrisi (§3 madde 7) aşar, önce §6 madde 14 sorulur. Her çekirdek diliminden sonra boyut ADR 0008'e yazılır.
- **Son doğrulama** (`main`, ifade dili, 24 Eylül, bulut konteyneri): `pnpm typecheck` temiz; `pnpm test` 725 test geçti, 10 atlandı (fixture kaydedicileri, istekle çalışan ölçümler); `pnpm build` başarılı; `pnpm rust:test` temiz (310 test, clippy `-D warnings`; veritabanı testleri sır olmadığı için atlandı); `cargo fmt --all -- --check` temiz; sayı yazımının derin koşusu (`NUMBER_ROUNDS=3000000`, release) temiz; `pnpm e2e` 134 denetim geçti (ifadeyle seçim, öznitelik hesabı, stil sınıflaması, worker eşitliği dahil), düşen üçü bu konteynerde düşen WebGPU denetimleri (§5; şeritten önceki `6505a6e` de aynı piksel sayılarıyla düşüyor). DXF ölçüsündeki doğrulamada alt ajan ayrıca örnek bir dosyayı ezdxf 1.4 ile denetledi (hata yok); ölçülü dosya da hatasız ve düzeltmesiz okunur, ezdxf'in yeniden çizdiği değerler KentOS'unkiyle aynıdır. Tam takımda `ellipse.test.ts`'in yoğun örnekleme denetimi makine yüklüyken bir kez 5 sn sınırını aştı; kendi süresi 30 sn yapıldı (`67c663d`).
- **Ölçüm:**
...
7. **style-core:** kullanıcı 24 Eylül'de başlattı (“5 maddeyi de yap”), dilim dilim aynı yöntemle (§4).
   ...
   - **Y2** stilin yerleştirme geometrisi (`style/geometry.ts`: çizgi boyunca işaret yeri, dalga yolları, alanın iç noktası, ağırlık merkezi, çizginin ortası).
   - **Y3** stil derleyicisi (sembol × geometri → çizim ilkelleri, `style/compile.ts`).
   - **Y4** SVG düzenleyicisinin geometrisi (yol işlemleri, düğümler, kenet; ayrı ve ilk kullanımda yüklenen WASM paketi, çünkü düzenleyici başlangıçta yüklenmez).
   - Başlangıç WASM'ı sınırın 0,45 KB altında: Y2 ve Y3'ten önce §6 madde 14 sorulur.
...
14. **Başlangıç WASM sınırı (350 KB gzip) stil motorunun taşınmasıyla aşılacak** (ifade dilinden sonra 349,6 KB). Seçenekler: sınırı yükseltmek; işlev adları bölümünü üretim paketinden atmak (bugün ~24 KB gzip: 349,6 → ~325 KB; bedeli, bir tuzağın yığın izinde adlar yerine numaralar); stil motorunu ayrı pakete koymak (çizim açılışta stil motorunu istediği için ilk yükü azaltmaz, yalnız ölçüyü böler).
