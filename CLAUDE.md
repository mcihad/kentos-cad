...
- **Üst katman** (`viewport/overlay.ts`, Canvas2D) şunları çizer: etiketler (`LabelStyle` ile; hangilerinin nerede çizileceğini geometri deposu söyler; ekranda üst üste binen etiket o görünümde çizilmez, yazı nesneleri ve ölçüler her zaman çizilir; görünüm, çizim ya da görünüş değişmedikçe etiketler hazır bir görüntüden kopyalanır, böylece fare hareketi etiketleri yeniden çizmez: 81 bin parselde 5–8,6 ms → 0,1 ms), tutamaçlar, kenet işareti, artı imleç, ölçek çubuğu, "K" kuzey oku ve araç önizlemeleri. GPU metni (SDF) gelene kadar yazılar buradadır.
...
- Kaydırma ve yakınlaştırma sırasında etiketler her karede depodan sorulup çizilir (81 bin parselde yakın görünümde ~4 ms, çoğu yazı çizimi); GPU metni (SDF) gelince bu da GPU'ya geçer.
...
- Çizim yazı tipi: yazı kutuları harf genişliklerinin toplamıdır, harf aralığı (kerning) sayılmaz; tabloda olmayan harf (Latin ve Latin Extended-A dışı) ortalama küçük harf sayılır. İşlem araçlarının köşe numarası yazısının yeri (`cornerTextAt`) ve SVG düzenleyicisinin yazı kutusu hâlâ harf başına ~0,55 em tahminidir. DXF dışa aktarma yazıları yine Standard (arial) stiliyle yazar; projenin çizim yazı tipi DXF'e taşınmaz.
