...
pnpm e2e:visual           # arayüzün görsel karşılaştırması (iki tema, dört vurgu, büyük yazı, pencereler) → scripts/e2e/visual/; --update kaynakları yeniden kaydeder
...
- **Çizim motoru:** varsayılan WebGL2'dir; WebGPU isteğe bağlıdır.
  ...
  - Başlangıç ekranı açılışta `prefs.startScreen` ile açılır; otomasyonda (`navigator.webdriver`: duman testi, ölçümler) açılmaz, `?start=1` zorlar, `?start=0` kapatır.
...
| **Uygulama ayarları**       | `app/state.ts` → `ctx.prefs`                                | `localStorage` `kentos.prefs.v1` | Yalnızca bu kullanıcı, tüm projeler | Tema, vurgu rengi (`prefs.accent`, varsayılan lacivert), arayüz yazı tipi (`prefs.uiFont`, varsayılan Plus Jakarta Sans), arayüz düzeni (klasik ya da şerit, `prefs.shell`), yazı boyutu (beş kademe), artı imleç, fare yardımcıları (imleç yanında giriş, bilgi kartı), kenet türleri ve yarıçapları, çizim motoru, çizim kalitesi (`prefs.renderQuality`: yüksek, dengeli, hızlı), çizgi kalınlığını gösterme (`prefs.lineWeights`, durum çubuğunda Kalınlık), açılışta başlangıç ekranı (`prefs.startScreen`), sembol boyutu (çizim ölçeğinde / ekranda sabit), **yeni proje varsayılan SRID'si (5256), çalışma modu ve çizim yazı tipi**; işlem araçlarının son değerleri (`kentos.processing.v1`); kullanıcının stil kitaplığı (`kentos.styles.v1`) |
| **Çalışma alanı yerleşimi** | `app/state.ts` → `ctx.ui`                                   | `localStorage` `kentos.ui.v1`    | Yalnızca bu kullanıcı               | Panel genişlikleri, araç kutusu konumu, sütun sayısı ve katlanan grupları, açık sekme, sağ dok sekmesi (Katmanlar/İşlemler), İşlemler görünümü ve katlanan kategoriler; şeridin açık sekmesi, daraltılmışlığı, hızlı erişime eklenenler, bölünmüş düğmelerin son seçimi ve şeritle birlikte araç kutusunun açıklığı                                                                                                                |
...
- **Hesap menüsü** (`app/calc.ts` komutları `calc.*`, pencereler `ui/calc/`, hesap Rust çekirdeğinde `geometry-core::survey`, cephe `model/geom/surveyCalc.ts`; Netcad'in Hesap menüsü): poligon hesabı (`POLIGON`: bağlı, kapalı, açık; kırılma açısı her istasyonda önceki noktadan sonrakine saat yönünde; bağlıda bitişte yöneltme isteğe bağlı; açı kapanma hatası “hesaplanan − bilinen” açılara eşit, koordinat kapanma hatası kenarlara uzunluklarıyla orantılı dağıtılır: pusula kuralı), kutupsal alım (`KUTUPSAL`: bakılan noktanın okumasıyla yöneltilmiş doğrultu okumaları; eğik uzunluk başucu açısıyla yataya iner, istasyon kotu, alet ve reflektör yüksekliğiyle nokta kotu), aplikasyon (`APLIKASYON`: semt, yatay uzunluk, bakılan noktadan saat yönünde açı), önden ve geriden kestirme (`ONDEN`, `GERIDEN`; geriden kestirmede tehlike dairesi reddedilir, yakını uyarılır; pencerede açıları gösteren kroki). Açılar proje açı biriminde (grad ya da derece), semt kuzeyden saat yönünde. Bilinen nokta çizimdeki nokta adıyla, “Y,X” yazılarak ya da çizimden gösterilerek verilir (bir noktaya kenetlenirse adı alınır). Ölçüler düzenlenebilir tabloya yazılır (Enter/↓ aşağı iner ve satır ekler; elektronik tablodan yapıştırılan satırlar sütunlara dağılır) ve yazıldıkça hesaplanır. Yeni noktalar seçilen katmana (poligon ve kestirmede “Poligon noktaları”) ad, `Ad`, `Tür` ve varsa `Z (m)` ile **tek geri alma adımıyla** eklenir; rapor sekmeyle ayrılmış olarak panoya kopyalanır. Girilenler oturum boyunca kalır: pencere kapanıp açılınca (çizimden nokta gösterirken de) olduğu gibi gelir. Hata sınırı (yönetmelik) uygulanmaz, kapanma hataları gösterilir.
...
- **Son dosyalar ve başlangıç ekranı** (`app/recentFiles.ts` → `ctx.files.recent`, `ui/start/`): açılan ve kaydedilen .kcad dosyaları (en çok 10, yenisi önde; aynı dosya `isSameEntry` ile tanınır) dosya tutamaçlarıyla IndexedDB'de (`kentos.files` / `recent`) durur; listeden açmak (`files.openRecent`) önce kaydedilmemiş değişikliği sorar, sonra tarayıcıdan dosya iznini yeniden ister; bulunamayan dosya listeden düşer. Saklanamayan tutamaç (bellek içi dosya) yalnız o oturumda kalır. Başlangıç ekranı (Dosya → Başlangıç ekranı, `file.start`; açılışta `prefs.startScreen`) yeni proje, dosya aç, bulut projesi, DXF içe aktar, çizime devam et ve son dosyaları gösterir; uygulama menüsü de son beş dosyayı listeler. İkisi de ilk kullanımda yüklenir.
...
- **`Entity`:** türler `point | line | polyline | polygon | circle | arc | ellipse | spline | xline | ray | text | dimension | hatch`.
  ...
  - `text`: seçim ve sınır kutusu, döndürülmüş metin kutusudur (`textBox`): genişlik harflerin projenin çizim yazı tipindeki ölçülmüş genişliklerinin toplamıdır (`geometry-core::text`; tablolar Chrome'da uygulamayla gelen yazı tipi dosyalarından `record-font-metrics.mjs` ile kaydedilir), geometri deposu yazı tipi değişince yazıları yeniden ölçer. Yay (`arc`) merkez, yarıçap ve radyan cinsinden `a0 → a1` açılarıyla, **her zaman saat yönünün tersine** tutulur; aynalama gibi yönü çeviren işlemler başlangıç ve bitişi değiştirerek bu kuralı korur. Ortak alanlar `layerId`, `color?` (yoksa katmana göre), `attrs: Record<string,string>` ve `label?` (çizimde gösterilen kısa metin: parsel no, nokta adı).
...
- **`RenderBackend` sözleşmesi** (`render/types.ts`): `init`, `resize`, `upload(SceneLayer)`, `remove(id)`, `render(FrameState)`, `dispose`.
  ...
  - **Taban görüntüsü:** arka uç alt katmanları ve belge katmanlarını ekran dışı bir hedefe çizip bir dokuda tutar; kare onu kopyalar, üst katmanları (`__hover`, `__sel`) üstüne çizer. Yalnız vurgu ya da seçim değiştiyse (`FrameState.keepBase`, denetleyicide `requestHighlight`) taban yeniden çizilmez; görünüm, katman yüklemesi, sıra, zemin ya da boyut değişince çizilir (`requestRender` tabanı kirletir). Büyük çizimde üzerine gelme her hareket olayında bütün parçaları yeniden çizdiriyordu (1 milyon parçada bulut makinesinde olay başına ~5 sn; artık ~5 ms). Kenar yumuşatma hedeflerin kendi 4× MSAA'sıdır (`backend.antialiased`): WebGL2 bağlamı `antialias: false` açılır, çünkü çok örneklemeli bir hedefe kopyalanamaz ve alfasız tuvale çok örneklemeli kopya biçim farkından reddedilir; taban ve kare tek örnekli dokulara çözülür, tam ekran bir üçgenle (`COPY_VS/FS`) çizim hedefine ve tuvale aktarılır. WebGPU aynısını yapar (`COPY_WGSL`).
...
- **Üst katman** (`viewport/overlay.ts`, Canvas2D) şunları çizer: etiketler (`LabelStyle` ile; hangilerinin nerede çizileceğini geometri deposu söyler; ekranda üst üste binen etiket o görünümde çizilmez, yazı nesneleri ve ölçüler her zaman çizilir; etiketler görünümü ve her yanda boyunun %35'i kadar payı kapsayan hazır bir görüntüden kopyalanır: fare hareketi etiketleri yeniden çizmez (81 bin parselde 5–8,6 ms → 0,1 ms), payın içinde kalan kaydırma görüntüyü kaydırarak kopyalar (kare başına p50 5,5–6,6 ms → 0,1 ms); yakınlaştırma, payı aşan kaydırma, çizim ya da görünüş değişikliği görüntüyü yeniden çizer, durulan görünüm tam piksel kaydırmada keskin kalır), tutamaçlar, kenet işareti, artı imleç, ölçek çubuğu, "K" kuzey oku ve araç önizlemeleri. GPU metni (SDF) gelene kadar yazılar buradadır.
...
- **Şerit** (`ui/ribbon/`: `Ribbon.ts` sekmeler, sığdırma, daraltma; `panels.ts` panel düzeyleri ve alanlı paneller; `controls.ts` düğmeler; `search.ts` Komut ara): ayrı JS ve CSS parçasıdır (§20), yalnız seçilince yüklenir; yüklenirken yerini aynı yükseklikte bir yer tutucu tutar, yüklenemezse klasik arayüze dönülür ve söylenir. Sekmeler ilk açılışta kurulur ve saklanır. Pencere daralınca paneller sağdan sola, her biri bir adım inerek küçülür (büyük → küçük etiketli → yalnız simge → tek düğme; genişlik kazandırmayan adım atlanır, Giriş'te Çizim ve Değiştir en son). Seçim varken bağlamsal **Seçim** sekmesi (sayı, türler, seçime uygulanan komutlar) çıkar; çalışan aracı içeren sekmeler amber nokta taşır; İşlemler sekmesi işlem kaydını ve modelleri izler. `Ctrl+F1` ya da sekmeye çift tık şeridi sekmelere daraltır; daraltılmışken sekme çizimin üstünde açılır, bir komut çalışınca kapanır. `Alt+Q` Komut ara (klasik arayüzde komut satırı). **Harf ipuçları** (`ui/ribbon/keytips.ts`, `view.keyTips`): Alt'a tek başına basıp bırakmak ya da `F6` sekmelerde harf, hızlı erişimde rakam gösterir; sekmenin harfleri sekmeyi açar ve düğmelerinde bir ya da iki harf çıkar, onlar düğmeyi çalıştırır (açılır düğmeler listelerini ilk öğe odakta açar, tek düğmeye katlanmış panel açılıp içindekiler harf alır); Esc bir düzey geri gider, tıklama ya da başka tuş kapatır. Harfler etiketlerden türetilir (Türkçe harfler katlanır), hiçbiri başka birinin başı değildir. Sağ tık düğmeyi hızlı erişime ekler. Şerit düğmesine tıklamak klavye odağını almaz (Enter son komutu yinelemeye devam eder). Şeritle birlikte araç kutusu kapalıdır (F9 ile açılır, ayrı hatırlanır). Geçerli özellik alanları (katman, renk, tip, kalınlık, ölçek) araç çubuğuyla ortaktır (`ui/toolbar/fields.ts`).
...
- Kenet uzak görünümde imlecin penceresine giren bütün nesnelere bakar: pencereden kutusuyla uzak köşe, kenar ve kesişim çiftleri hesaplanmadan atlanır (sonuç bütün çiftleri deneyenle bit bit aynı; `store/snap.rs`), 50 bin parselde tüm verinin dörtte biri ölçekte kenet 17,8 → 3,9 ms, 1 milyon parçalık eşyükseltide ~4 ms. Daha da uzakta nesne sayısıyla büyür.
- Yakınlaştırma sırasında etiketler her karede depodan sorulup çizilir (81 bin parselde yakın görünümde ~4–11 ms, çoğu yazı çizimi); kaydırma hazır görüntüyü kaydırır, ama payı aşınca görüntü (görünümün ~2,9 katı alan) tek karede yeniden çizilir: yakın görünümde ~29 ms, yaklaşık üçte bir ekranlık yolda bir kez. GPU metni (SDF) gelince bu da GPU'ya geçer.
- Katman ağacının biçimi değişince (katman eklenir, adı değişir, grup açılır ya da kapanır) pencerenin satırları baştan kurulur, kimliğe göre yeniden kullanılmaz: sanallaştırmadan sonra bu ~50 satırdır (aşağıda); daha büyük ağaçlarda da sabit kalır, yeniden kullanmak kalan kazançtır. Koordinat listesi yalnız kaydırma penceresindeki satırları kurar (`ui/widgets/VirtualRows.ts`; 10 000 noktada 37 satır); komut geçmişi ve uyarılar her iletide listeyi baştan kurmaz, yeni satırı ekler (en çok 500 satır). Öznitelik tablosu henüz yok; gelince aynı bileşenle kurulmalı.
...
| `wasm/calls/reference.test.ts` (Hesap) | Bağlı, kapalı ve açık poligon, kutupsal alım, aplikasyon, önden ve geriden kestirme bağımsız referansa (`reference-calls.json`: 60 basamaklı seriler, geriden kestirmede Newton yöntemi) göre 1e-8 m içinde; Rust birim testleri (`geometry-core/src/survey/tests.rs`): noktalardan üretilen ölçülerin noktaları geri vermesi, konan açı ve uzunluk hatalarının kapanma hatası olarak çıkması, pusula kuralı, tehlike dairesi, giriş denetimi |
...
- **Görsel karşılaştırma** `apps/web/scripts/e2e/visual.mjs` (`pnpm e2e:visual`): klasik arayüz ve şerit (koyu, açık), dört vurgu rengi, büyük yazı boyu, Uygulama ayarları, başlangıç ekranı ve uygulama menüsü bölgelerinin ekran görüntüsü `scripts/e2e/visual/*.png` ile sayfanın kendi canvas'ında piksel piksel karşılaştırılır (bağımlılık yok); bir bölgede piksellerin %0,4'ünden fazlası farklıysa başarısızdır ve fark kırmızıyla `out/visual-*-diff.png`'ye yazılır. Çizim alanı dışarıda kalır (GPU'ya bağlı). Kaynaklar bulut konteynerinde kaydedildi; yazı tipi işleme sistemden sisteme biraz değiştiği için başka makinede önce `--update` ile kaydedilir. Bilinçli bir görünüş değişikliği de `--update` ile kaydedilir ve farkı okunur.
- Tıklama noktaları ekrandan tahmin edilmez; dünya koordinatından `camera.worldToScreen` ile hesaplanır.
- **Fixture kaydedicileri** uygulamanın yolundan kaydeder ve `apps/web/scripts/fixtures/record-*.test.ts`'tedir (`record-expression`: ifade yanıtları; `record-style`: stil derleyicisinin sembol ve katman yanıtları; `record-svg`: SVG çekirdeğinin yanıtları, tablonun her işlemi `style/svg/cases/calls.ts` `CALLS`'tan; `record-collation`: Türkçe sıralama tablosu, Node'un ICU'sundan `crates/shared/style-core/src/js/collation_tr.rs`; tarayıcı isteyen `record-font-metrics.mjs`: çizim yazı tiplerinin harf genişlikleri, `CHROME_BIN=… node scripts/fixtures/record-font-metrics.mjs` → `crates/shared/geometry-core/src/text/metrics.rs`): `GOLDEN_WRITE=1 pnpm -C apps/web exec vitest run scripts/fixtures/<ad>.test.ts` (değişen dosya bilinçli bir golden değişikliğidir, farkı okunur). Dilden bağımsız referans üreticileri kökten çalışır: `python3 scripts/fixtures/<ad>.py`.
...
- taban görüntüsü (§4.9): alt katmanlar ve belge katmanları `base` dokusuna çözülür, kare onu tam ekran üçgenle (`COPY_WGSL`, GLSL'de `COPY_VS/FS`) kopyalayıp üst katmanları çizer
...
1. **CAD çekirdeği:**
   ...
   - **Netcad çizim eşdeğerliği (referans Netcad'dir; AutoCAD ikincil ölçüttür. Bağlayıcı hedef: çizim kusursuz olmadan başka işe geçilmez):**
     ...
     - *Netcad'e özgü, var (Hesap menüsü):* poligon hesabı (bağlı, kapalı, açık; pusula kuralı), kutupsal alım (takeometri, kotlarla), aplikasyon, önden ve geriden kestirme.
     - *Netcad'e özgü, eksik:* alanı verilen alana göre bölme (ifraz, topolojiyle); sembol ve blok yerleştirme; klotoid (spiral) nesnesi; en küçük kareler dengelemesi, nivelman ve trigonometrik nivelman hesapları, nokta dönüşümleri (Helmert, afin).
...
- Hesap menüsü: yönetmelik hata sınırları uygulanmaz (kapanma hataları gösterilir, değerlendirme kullanıcının); poligonda dengeleme pusula kuralıyladır (en küçük kareler yok), bağlı poligonun iki ucunda yalnız birer yöneltme olur; açılar ondalık grad ya da derece yazılır (derece-dakika-saniye girişi yok); kutupsal alımda yer eğriliği ve ışın kırılması uygulanmaz, bir pencerede tek istasyon vardır; geriden kestirme yalnız üç noktalıdır (fazla ölçü yok). Poligon kenarları çizgi olarak çizilmez, yalnız noktalar eklenir. Sonuçlar §23'ün kadastral kesinlik kapısından geçmez (f64, bağımsız referansla 1e-8 m içinde).
- Uygulama kodla üretilen örnek projeyle açılıyor (`model/sampleProject.ts`); Yeni proje boş çizim açar. Son dosyalar yalnız bu tarayıcıda tutulur (cihazlar arası eşitlenmez) ve yalnız .kcad dosyalarıdır (içe aktarılan DXF ya da koordinat listesi girmez). Dosya ikili parçasız, tek JSON'dur.
...
- Katman çizgi kalınlığı (`lineWeight`) kâğıt milimetresidir ve çizim ölçeğinde (ya da “Ekranda sabit” sembollerde ekranda) çizilir; AutoCAD'in LWT'si gibi durum çubuğundaki “Kalınlık” düğmesi (`view.lineWeights`, `prefs.lineWeights`) kapatınca katmanların basit görünüşü ince çizilir. Kitaplık sembollerinin ve işleyicilerin kendi kalınlıkları bu düğmeyle incelmez.
...
- Şerit: harf ipuçları Alt'a tek başına basıp bırakınca açılır; Alt+harf tarayıcının menülerine gittiği için Office'teki gibi Alt basılıyken harf yazılamaz (harfler Alt bırakıldıktan sonra), bazı işletim sistemlerinde Alt tarayıcıya gidiyorsa `F6` kullanılır. Harfler etiketlerden her açılışta yeniden türetilir, sabit bir tabloya yazılmaz. Seçim sekmesinin içeriği seçilen türlere göre daralmıyor (alan işlemleri her zaman görünür). Hızlı erişim yalnız ekleme ve kaldırmayı biliyor, sıralama yok. Panel genişlikleri bir kez ölçülür; yazı ölçeği ya da yazı tipi değişince yeniden ölçülür, başka içerik değişikliği (ör. uzayan bir etiket) bir sonraki sekme kurulumuna kalır. Bölünmüş düğmelerin son seçimi çalışma alanı yerleşiminde (`ui.ribbonSplits`) saklanır; yöntemlerin kendi simgesi yok (düğme aracın simgesini gösterir, yöntem ipucunda yazar); yöntem aracı istemde seçeneğe basılmış gibi başlatır, bu yüzden aracın ilk adımında geçerli bir seçenek olmalıdır.
- Çizim yazı tipi: yazı kutuları harf genişliklerinin toplamıdır, harf aralığı (kerning) sayılmaz; tabloda olmayan harf (Latin ve Latin Extended-A dışı) ortalama küçük harf sayılır. İşlem araçlarının köşe numarası yazısı da projenin çizim yazı tipiyle ölçülür (`cornerTextAt`, `DefaultsContext.drawingFont`); SVG düzenleyicisinin yazıları Arial olduğu için onların kutusu Arimo tablosuyla ölçülür (serif yazılar da; Times biraz daha dardır). DXF dışa aktarma yazıları yine Standard (arial) stiliyle yazar; projenin çizim yazı tipi DXF'e taşınmaz.
...
- Açılış: sistem sembol kitaplığı, örnek proje ve gösterim kataloğu ayrı bir parçadır (`app/startContent.ts`, WASM çekirdeğiyle paralel iner); ayar, işlem aracı ve bulut pencereleri, başlangıç ekranı ve WebGPU arka ucu ilk kullanımda yüklenir (`render/webgpu/support.ts` desteği yüklemeden söyler). İlk sayfanın JS'i 844 → 539 KB (gzip 263 → 170 KB), soğuk açılış 865 → 808 ms (`docs/perf/README.md`). Uygulama hâlâ örnek projeyle açıldığı için kitaplık açılışta iner; boş projeyle açılan bir kurulumda kitaplığı sembol istenince yüklemek sıradaki adımdır. Derleme her `.wasm`, `.js` ve `.css`'in yanına Brotli ve gzip kopyasını yazar, `vite preview` onları gönderir (`vite.config.mjs` `kentosCompress`): soğuk açılışta aktarılan 1 536 → 683 KB, WASM 1 110 → 298 KB. Üretimdeki statik sunucu bu kopyaları göndermelidir (nginx `brotli_static`/`gzip_static`, Caddy `precompressed`; ayrıntı `docs/perf/README.md`); depoda üretim sunucusu yoktur.
...
apps/web/src/
  ...
  app/                       Kompozisyon kökü
    ...
    fileIO.ts                DocumentFiles: yerel .kcad kaydet/farklı kaydet/aç, son dosyadan açma, içe aktarılacak dosyanın seçimi, dosya seçici (tarayıcı ya da test için bellek içi)
    recentFiles.ts           RecentFiles: son açılan ve kaydedilen dosyalar, tutamaçlarıyla IndexedDB'de
    calc.ts                  Hesap menüsünün komutları (poligon, kutupsal alım, aplikasyon, kestirmeler); pencereleri ilk kullanımda yükler
  ...
  model/                     Belge, varlıklar, geometri, katmanlar, seçim, proje ayarları, örnek proje, standart katman ağacı (standardLayers), boş yeni proje (newProject), tek hesap kaynağı bekçisi (singleSource.test.ts)
    ...
    geom/                    Geometri çekirdeğinin cepheleri (Rust'ta): afin, yay, bulge, kesişim, elips, eğri; öteleme, teğet daire, düzlem bindirme ve alan cebiri, şekiller, ölçmecilik, Hesap menüsünün hesapları (surveyCalc), tarama, ölçü (+ testler)
  ...
  ui/
    ...
    start/                   Başlangıç ekranı (StartScreen, `styles/start.css`) ve son dosya satırları (recentList, `styles/recent.css`; uygulama menüsü de kullanır); ilk kullanımda yüklenir
    ...
    widgets/                 Genel parçalar (menü, açılır liste, ağaç, özellik ızgarası, pencere, onay penceresi `confirm.ts`, kontroller, uzun listeler için `VirtualRows`)
    cloud/                   Buluta giriş, bulut projeleri (aç, yükle), yeniden adlandırma ve silme (ProjectActions), kayıt çakışması pencereleri
    calc/                    Hesap pencereleri (ilk kullanımda yüklenir, `styles/calc.css` ile): common (bilinen nokta alanı, ölçü tablosu, sonuç tablosu, noktaları çizime ekleme, rapor), TraverseDialog, PolarDialog, StakeoutDialog, IntersectionDialog
...
crates/                      Rust kütüphaneleri, rolüne göre (Cargo çalışma alanı, kök Cargo.toml)
  shared/                    Platformdan bağımsız: web (WASM ile), sunucu ve ileride masaüstü (wgpu) uygulaması aynen kullanır; DOM, tarayıcı, veritabanı ve ağ bilmez
    geometry-core/           Saf analitik geometri (f64), jsmath (JavaScript sayı anlamı, libm), predicates (kesin yön yüklemi, §23.3), api (çağrı tablosu, JSON yazıcı), store (geometri deposu: R-ağacı, seçme, kenet, paketli okuyucu ve yazıcı, paketli dönüşüm, işlem sorguları), survey (Hesap menüsü: poligon, kutupsal alım, aplikasyon, kestirmeler), processing (köşe numaralama, kenar ölçüleri), tools (nokta girişi, nesne izleme, araçların yapı hesapları), triangulate ve §23 sayısal politika (rust_decimal); clippy.toml (std aşkın işlevleri yasak); tests/ (golden, çağrı fixture'ları, depo, bağımsız referans, sayısal)
...
apps/                        Çalıştırılabilir uygulamalar
  ...
  web/                       Tarayıcı uygulaması (pnpm paketi `@kentos/web`): src/ (yukarıdaki ağaç), index.html, public/, vite.config.mjs (/v1 isteklerini yerel API'ye ileten eklenti, dev ve preview), tsconfig.json, package.json
    scripts/e2e/             Başsız Chrome duman testi (cdp.mjs sürücü, smoke.mjs senaryo), görsel karşılaştırma (visual.mjs, kaynak görüntüler visual/) ve bulut senaryosu (cloud.mjs)
