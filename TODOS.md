...
- [ ] `UI-11` İlk desktop shell'i mevcut web menü/komut envanterinden kur: proje aç/kaydet, ribbon, layer tree, properties, status, command input, settings ve cloud durumu. — **25 Eylül, kısmen:** `apps/desktop` ([ADR 0017](docs/adr/0017-desktop-shell.md)).
  ...
  - Taşınan komut 56/167 (26 Eylül; `apps/desktop/ported.json`, envanterde masaüstü sütunu).
  ...
  - Kalanlar: çalışma modunun şeridi ve menüleri süzmesi (`workspace.*`); GeoJSON ve Shapefile; çizim alanında yazı, ölçü ve öbür çizim araçları (elips, eğri, halka, yardımcı çizgi …); ötele, dizi, hizala ve alan araçları.
