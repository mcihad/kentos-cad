...
- [ ] `UX-06` Enter, Esc, Tab/Shift+Tab, Backspace, sağ tık, çift tık, son komutu tekrarla, undo last point ve polygon kapatma anlamını platformlar arasında fixture'la doğrula. — **25 Eylül, kısmen:** anlamlar ADR 0018'de, `polygon-keys` ve `polygon-close` izlerinde; web geçiyor. Sahibin kararları uygulandı: ilk köşeye dönmek alanı kapatır, Boşluk Enter'dır, komut çalışırken Ctrl+Z en yeni adımı geri alır, değer için tek alan kalır. Tab artık değeri silmiyor. Masaüstü de geçiyor (ADR 0021): `polygon-keys` ve `polygon-close` native oynatıcıda üç varyantta. Kalan: Shift+Tab (iki alan gelirse, `UX-08`). 26 Eylül: çizgi ve çoklu çizgide de (`line-chain`, `polyline-arc`: Geri, Kapat, Ctrl+Z sırası, sağ tık). Masaüstünde komut satırında Esc, öneri listesi açıkken de yazılanı siliyor (ADR 0018'e uyum, KentOS UI `escape_clears`; ADR 0027). 26 Eylül: onay almayan araçta (Kaydır, Pencere yakınlaştır) Enter son komutu yineler, Kaydır son komut sayılmaz; `tool.repeat` masaüstünde de (`navigation`, [ADR 0056](docs/adr/0056-desktop-clipboard-and-view-tools.md)). İz biçimi `Shift+` akorlarını ve `viewCenter` beklentisini aldı. Tam ekrandan Esc yalnız iptal edilecek komut, değer ve seçim yokken çıkar; tarayıcıda her zaman çıkar ([ADR 0058](docs/adr/0058-desktop-panels-and-view-commands.md)). 26 Eylül: masaüstünde sağ tuş web'inki gibi: kısa tık onay alan komutta Enter, öbür durumda boşta menüsü; 300 ms basılı tutmak komut ya da boşta menüsü; Shift ile tek seferlik kenet menüsü; tek seferlik kenet sonraki sol basışta düşer ([ADR 0059](docs/adr/0059-desktop-drawing-menus.md)).
...
- [ ] `UI-11` İlk desktop shell'i mevcut web menü/komut envanterinden kur: proje aç/kaydet, ribbon, layer tree, properties, status, command input, settings ve cloud durumu. — **25 Eylül, kısmen:** `apps/desktop` ([ADR 0017](docs/adr/0017-desktop-shell.md)).
  ...
  - Pano: Kes, Panoya kopyala, Yapıştır ve Özgün koordinatlara yapıştır (oturumun panosu, sistem panosu yok, belgeden yazar); görünüm: Kaydır, Pencere yakınlaştır, Seçime yakınlaştır, Son komutu yinele ([ADR 0056](docs/adr/0056-desktop-clipboard-and-view-tools.md)).
  - Çizim alanının sağ tık menüleri (boşta, komut, tek seferlik kenet) ve tek seferlik kenet ([ADR 0059](docs/adr/0059-desktop-drawing-menus.md)).
  - Katman ağacı çizimdeki seçimi izler: seçili nesnelerin katmanları seçili görünür, grupları açılır, etkin katman değişmez (sahibin isteği; sanal satırlar). Satırları web'inki gibi: göz ve kilit, etkin katman çubuğu, çift tıkla etkin yapma, renk kutusunun menüsü, web'in sağ tık menüsü, yerinde yeniden adlandırma. Kalan: ağacın klavyesi.
  - Alt panel (F2: komut geçmişi, koordinat listesi, uyarılar; Geçmişi temizle, Paneli kapat, üst kenarı sürüklenerek boy), durum çubuğu arayüzün yazı boyu ve yazı tipiyle; F4 katman ve özellikler panellerini birlikte gizler ve düzeniyle geri getirir; tam ekran (sekme satırında düğmesi); komut arama (Alt+Q: komut satırının listesi, masaüstünde çalışmayan komutlar soluk); Koordinat sistemi… (durum çubuğundan da); sunucu denetimi ([ADR 0058](docs/adr/0058-desktop-panels-and-view-commands.md)). Koordinat listesinin alanı iki platformda da düzeldi: nesnenin kendi alanı, kenarlar halkasında.
  - Taşınan komut 105/167 (26 Eylül; `apps/desktop/ported.json`, envanterde masaüstü sütunu).
  ...
  - Çizimin yazıları (yazı nesnesi, ölçü değeri, etiket), ölçü çizgileri ve yardımcı çizgiler çiziliyor ([ADR 0055](docs/adr/0055-desktop-drawing-text.md)).
  - Çizim araçları, 3. tur: elips, eğri, yardımcı çizgi, ışın, paralel çizgi, dik in, dik çık, halka, revizyon bulutu, kot noktası, böl; kendi komutu olmayanlar `cad.entities.create` v1'den yazar ([ADR 0057](docs/adr/0057-desktop-drawing-tools-3.md)).
  - Yazı aracı ve çizimin üstündeki yazı kutusu; çift tıkla yazı ve ölçü değeri düzenleme ([ADR 0060](docs/adr/0060-desktop-text-tool.md)).
  - Ölçülendirme: hizalı, doğrusal (kilitli ya da imleçten doğrultu), iki kenar ya da köşeden açı, yarıçap ve çap; `dimensions` izi iki platformda; komut satırı değerleri ve seçenekleri yazıldığı gibi yankılar ([ADR 0061](docs/adr/0061-desktop-dimension-tool.md)).
  - Tarama: kapalı nesneyle ya da çizgilerle bölge, adalar, nesneyle seçilen sınır katmanı, dört desen; `hatches` izi iki platformda; `cad.entities.create`'in `hatch` işlemi ("Tarama") ([ADR 0062](docs/adr/0062-desktop-hatch-tool.md)).
  - Kalanlar: alan araçları; ölçme, parsel, aplikasyon ve ifraz araçları; hesaplar (`calc.*`), işlemler, stil düzenleyicileri, bulut komutlarının kalanı, sağ tık menüleri.
- [ ] `UI-12` Linux Wayland/X11, Windows ve macOS için pencere, DPI, IME, clipboard, dosya association ve erişilebilirlik matrisi oluştur; ilk geliştirme platformunu diğerlerinin mimari yasağına dönüştürme. Masaüstünün panosu şimdilik uygulama içi (ADR 0056); sistem panosu bu matrisle birlikte karara bağlanacak.
...
- [ ] `REN-12` Stil/etiket hesabında mevcut style-core'u kullan; metin shaping, font fallback, Türkçe glyph, atlas eviction, hinting ve küçük yazı okunaklılığını platform bazında doğrula. — **26 Eylül, masaüstünde kısmen** ([ADR 0055](docs/adr/0055-desktop-drawing-text.md)): yazı nesneleri, ölçü değerleri ve etiketler web'in `drawLabels` kurallarıyla, ortak deponun `labels` sorgusundan, projenin yazı tipiyle (web'in WOFF2'lerinden üretilen 22 TrueType yüz, `scripts/fonts/drawing_fonts.py`), Türkçe glifler ve haleyle; ölçü çizgileri ve kırpılmış yardımcı çizgiler sahnede. Açık: SDF/atlas metni (iki platform), eğik yüzü olmayan ailelerde eğik, yazının katman sırası, küçük yazı okunaklılığının platform ölçümü.
