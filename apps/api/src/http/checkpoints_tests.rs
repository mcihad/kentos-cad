//! Checkpoints over HTTP (docs/adr/0034): made and restored through the
//! project's command route, listed and downloaded through their own; a
//! stranger finds both routes missing as for no project.

use axum::http::{StatusCode, header};
use kentos_application::admin;
use kentos_contracts::{
    CheckpointChange, CommandEnvelope, ProjectCheckpoints, ProjectDuplicated, ProjectInfo,
    TenantRole,
};
...
async fn a_checkpoint_is_made_listed_and_downloaded_over_http() {
    ...
    // Restored as a new project through the command route.
    let back = envelope(
        &tenant,
        &project,
        "project.checkpoint.restore",
        serde_json::json!({ "checkpointId": made.checkpoint.id }),
    );
    let (status, _, body) = send(
        &router,
        json_req("POST", &format!("{base}/commands"), &ayse, back),
    )
    .await;
    assert_eq!(status, StatusCode::OK, "{}", String::from_utf8_lossy(&body));
    let restored: ProjectDuplicated = serde_json::from_slice(&body).unwrap();
    assert_eq!(restored.project.name, "Ada 3 (Teslim)");
