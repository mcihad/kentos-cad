import init, { formatsVersion, readCoords, writeCoords } from './pkg/kentos_formats_wasm.js';
...
scope.onmessage = (e) => {
  ...
  queue = queue.then(async () => {
    ...
    try {
      if (m.op === 'readCoords') {
        const json = own(readCoords(new Uint8Array(m.bytes), JSON.stringify(m.options)));
